...
TeapotGeometry::TeapotGeometry(int size, int segments, bool bottom, bool lid, bool body, bool fitLid,
                               bool blinn) {
    // 32 * 4 * 4 Bezier spline patches
    std::vector<size_t> teapotPatches = {
    /*rim*/
    ...
    float blinnScale = 1.3;

    // scale the size to be the real scaling factor
    float maxHeight = 3.15 * ( blinn ? 1 : blinnScale );
    ...
    std::vector<unsigned> indices( numTriangles * 4 );
    ...
    std::vector<float> g( numVertices * 3 );

    std::vector<float> sp( 4 );
    std::vector<float> tp( 4 );
    std::vector<float> dsp( 4 );
    std::vector<float> dtp( 4 );

    // M * G * M matrix, sort of see
    // http://www.cs.helsinki.fi/group/goa/mallinnus/curves/surfaces.html
    std::vector<Matrix4> mgm( 3 );
    std::vector<float> vert( 3 );
    std::vector<float> sdir( 3 );
    std::vector<float> tdir( 3 );
    ...
    auto vsdir = Vector3();
    auto vtdir = Vector3();
    auto mst = Matrix4().copy(ms);
    mst.transpose();

    // internal function: test if triangle has any matching vertices;
    // if so, don't save triangle, since it won't display anything.

    // lambda,if any vertex matches, return false
    auto notDegenerate = [&]( int vtx1, int vtx2, int vtx3 ){
    ...
    for ( int surf = minPatches; surf < maxPatches; surf ++ ) {
        ...
        if ( lid || surf < 20 || surf >= 28 ) {
            ...
            for ( int sstep = 0; sstep <= segments; sstep ++ ) {
                ...
                auto s = (float)sstep / segments;

                for ( int tstep = 0; tstep <= segments; tstep ++ ) {

                    auto t = (float)tstep / segments;
            ...
            for ( int sstep = 0; sstep < segments; sstep ++ ) {
                ...
                for ( int tstep = 0; tstep < segments; tstep ++ ) {
                    ...
                    auto v1 = surfCount * vertPerRow * vertPerRow + sstep * vertPerRow + tstep;
                    auto v2 = v1 + 1;
                    auto v3 = v2 + vertPerRow;
                    auto v4 = v1 + vertPerRow;
    ...
    this->index = BufferAttribute<unsigned>::create( indices, 1);
    setAttribute(AttributeName::position,BufferAttribute<float>::create(vertices, 3));
    setAttribute(AttributeName::normal,BufferAttribute<float>::create(normals, 3));
    setAttribute(AttributeName::uv, BufferAttribute<float>::create(uvs, 2));

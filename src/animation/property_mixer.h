...
//
        ...
        this->buffer = std::vector<bool>( valueSize * 5 );
    ...
    void _mixBufferRegion( std::vector<bool> buffer, size_t dstOffset, size_t srcOffset, float t, size_t stride ) {
    ...
    // Use the regular mix function and for additive on these types,
    // additive is not relevant for non-numeric types
    void _mixBufferRegionAdditive( std::vector<bool> buffer, size_t dstOffset, size_t srcOffset, float t, size_t stride ) {
        this->_mixBufferRegion( buffer, dstOffset, srcOffset, t, stride );
    }

    void _setIdentity() {

        auto startIndex = this->_origIndex * this->valueSize;
        auto targetIndex = this->_addIndex * this->valueSize;

        for ( size_t i = 0; i < this->valueSize; i ++ ) {

...
public:
    ...
    FrenetFrames computeFrenetFrames( int segments, bool  closed ) {
        ...
        for ( int i = 1; i <= segments; i ++ ) {
            normals.push_back(normals[ i - 1 ].clone());
            //binormals[ i ] = binormals[ i - 1 ].clone();
            binormals.push_back(binormals[ i - 1 ].clone());

            vec.crossVectors( tangents[ i - 1 ], tangents[ i ] );
            if ( vec.length() > math_number::EPSILON ) {
                vec.normalize();
                const float theta = std::acos( math::clamp<float>( tangents[ i - 1 ].dot( tangents[ i ] ), - 1, 1 ) ); // clamp for floating pt errors
        ...
        if ( closed == true ) {
            ...
            float theta = std::acos( math::clamp<float>( normals[ 0 ].dot( normals[ segments ] ), - 1, 1 ) );
            theta /= segments;

...
Geometry& Geometry::fromBufferGeometry(BufferGeometry& geometry){
    ...
    BufferAttribute<float>::sptr positions = geometry.getAttribute(AttributeName::position);
    BufferAttribute<float>::sptr normals = geometry.getAttribute(AttributeName::normal); // as BufferAttribute<float>)["array"] as float[];
    BufferAttribute<float>::sptr colors = geometry.getAttribute(AttributeName::color);// as BufferAttribute<float>)["array"] as float[];
    BufferAttribute<float>::sptr uvs = geometry.getAttribute(AttributeName::uv);// as BufferAttribute<float>)["array"] as float[];
    BufferAttribute<float>::sptr uvs2 = geometry.getAttribute(AttributeName::uv);//as BufferAttribute<float>)["array"] as float[];

...
//
#ifndef THREE_CPP_GEOMETRY_H
#define THREE_CPP_GEOMETRY_H
...
namespace std {
    template<> struct hash<AttributeNameKey> {
        typedef AttributeNameKey argument_type;

        std::size_t operator()(const argument_type& s) const {
            auto h = threecpp::EnumHash{}(s.first);
            threecpp::hash_combine(h, s.second);
            return h;
        }
    };
}
...
public:
    using OnDispose = threecpp::Signal<void(Geometry*)>;
    ...
    using sptr = std::shared_ptr<Geometry>;

    static sptr create() {
        return std::make_shared<Geometry>();
    }

    static sptr create(const Geometry& source) {

...
static void drawStuff() {
    ...
    visibleCamera->aspect = aspect;;
    visibleCamera->fov = 25;
    //注意，由于我们使用右手性的投射矩阵，故而far值一定要小于near值
    visibleCamera->near = -0.5;
    visibleCamera->far = -2;
...
//    teapotModel->Render(1.0, 0.055, 0.027);
    ...
    cameraModel->setPerspectiveCamera(mainCamera);
    cameraModel->Render(1.0, 0.055, 0.027);
...
static void mouse(const float x, const float y,long tapCount,bool press) {
    g_mouseClickX = x;
...
static void initGeometry() {
    ...
    cameraModel.reset(new CameraModel());
    cameraModel->Init();

...
float ImprovedNoise::noise(float x, float y, float z) {
    ...
    const float xMinus1 = x - 1, yMinus1 = y - 1, zMinus1 = z - 1;

    const float u = fade( x ), v = fade( y ), w = fade( z );

    const float A = _p[ X ] + Y, AA = _p[ A ] + Z, AB = _p[ A + 1 ] + Z, B = _p[ X + 1 ] + Y, BA = _p[ B ] + Z, BB = _p[ B + 1 ] + Z;

    return math::lerp( w, math::lerp( v, math::lerp<float>( u, grad( _p[ AA ], x, y, z ),
                                   grad( _p[ BA ], xMinus1, y, z ) ),
                                math::lerp<float>( u, grad( _p[ AB ], x, yMinus1, z ),
                                grad( _p[ BB ], xMinus1, yMinus1, z ) ) ),
                       math::lerp( v, math::lerp<float>( u, grad( _p[ AA + 1 ], x, y, zMinus1 ),
                                grad( _p[ BA + 1 ], xMinus1, y, zMinus1 ) ),
                                   math::lerp<float>( u, grad( _p[ AB + 1 ], x, yMinus1, zMinus1 ),

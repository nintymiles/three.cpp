...
#ifndef THREE_CPP_BLACK_HOLE_SHADER_H
#define THREE_CPP_BLACK_HOLE_SHADER_H

#include "shader_material.h"
#include "uniform_values.h"
#include "vector2.h"

struct BlackholeShaderData {
    static ShaderMaterial::sptr blackHoleMainShader(){
        ShaderMaterial::sptr shader = std::make_shared<ShaderMaterial>();

        UniformValues::sptr uniforms =std::make_shared<UniformValues>();
        uniforms->set<CubeTexture::sptr>("galaxy", nullptr);
        uniforms->set<Texture::sptr>("colorMap", nullptr);
        uniforms->set("amount",.005f);
        uniforms->set("time",.0f);
...
#endif //THREE_CPP_BLACK_HOLE_SHADER_H
